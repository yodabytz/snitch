...
EMAIL_ADDRESS = "abuse_report@your_domain.com" # Replace with your email
EMAIL_PASSWORD = "your_password" # Replace with your email password
SMTP_SERVER = "smtp.your_server.com" # Replace with your SMTP server
SMTP_PORT = 587
...
def ip_in_whitelist(ip_address):
    for label, entries in WHITELIST.items():
        for entry in entries:
            try:
                if "/" in entry:
                    if ipaddress.IPv4Address(ip_address) in ipaddress.IPv4Network(entry):
                        return label
        ...
        with open(FAIL2BAN_LOG, 'r') as file:
            ...
            while True:
                ...
                if match:
                    ...
                    label = ip_in_whitelist(ip_address)
                    if label:
                        log_action(f"Ban detected for IP: {ip_address} in jail: {jail_name} (Whitelisted: {label})")
                        continue
